...
public class OrderDomainServiceImp implements OrderDomainService {
	...
	private void setOrderProductInformation(Order order, Restaurant restaurant) {
		order.getItems().forEach(orderItem -> restaurant.getProducts().forEach(restaurantProduct -> {
			Product currectProduct = orderItem.getProduct();
			if (currectProduct.equals(restaurantProduct)) {
				currectProduct.updateWithConfirmendNameAndPrice(restaurantProduct.getName(),
